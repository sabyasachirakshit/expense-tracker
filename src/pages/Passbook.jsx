...
export default function Passbook({ data, setData }) {
  ...
  return (
    <div className="h-full bg-gray-50 dark:bg-gray-900 flex flex-col overflow-hidden">
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex flex-col items-end gap-2 shrink-0">
                  ...
                  <div className="flex items-center gap-1">
                    <button onClick={() => setViewRecord(record)} className="p-1.5 rounded-lg bg-gray-100 dark:bg-gray-700 text-gray-400 dark:text-gray-500" title="View details">
      ...
      {viewRecord && (() => {
        const vAcc     = viewRecord.type !== 'transfer' ? getAccount(viewRecord.accountId) : null
        const vFromAcc = viewRecord.type === 'transfer' ? getAccount(viewRecord.fromAccountId) : null
        const vToAcc   = viewRecord.type === 'transfer' ? getAccount(viewRecord.toAccountId) : null
        const vTags    = getTags(viewRecord.tagIds)
        const amtColor = viewRecord.type === 'expense' ? 'text-red-500' : viewRecord.type === 'income' ? 'text-green-500' : viewRecord.type === 'adjustment' ? 'text-purple-500' : 'text-blue-500'
        return (
          <div className="absolute inset-0 z-50 flex flex-col justify-end bg-black/50" onClick={() => setViewRecord(null)}>
            <div className="bg-white dark:bg-gray-900 rounded-t-3xl shadow-2xl max-h-[85vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
              {/* Drag handle + fixed top */}
              <div className="shrink-0 px-5 pt-4 pb-3 border-b border-gray-100 dark:border-gray-800">
                <div className="w-10 h-1 bg-gray-200 dark:bg-gray-700 rounded-full mx-auto mb-4" />
                <div className="flex items-start gap-3 mb-3">
                  <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full capitalize shrink-0 mt-0.5 ${TYPE_BADGE[viewRecord.type]}`}>
                    {viewRecord.type}
                  </span>
                  <h3 className="text-base font-bold text-gray-800 dark:text-white leading-snug">{viewRecord.title}</h3>
                </div>
                <p className={`text-3xl font-bold ${amtColor}`}>{fmtAmt(viewRecord.amount, viewRecord.type)}</p>
              </div>
              ...
              <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
                ...
                <div className="space-y-0">
                  <div className="flex justify-between items-center py-2.5 border-b border-gray-100 dark:border-gray-800">
                    ...
                    <span className="text-xs font-semibold text-gray-700 dark:text-gray-200 text-right max-w-[60%]">
                      {viewRecord.type === 'transfer'
                        ? `${vFromAcc?.name ?? '?'} → ${vToAcc?.name ?? '?'}`
                        : vAcc?.name ?? '—'}
                    </span>
                  </div>
                  <div className="flex justify-between items-center py-2.5">
                    <span className="text-xs text-gray-400 dark:text-gray-500">Date &amp; Time</span>
                    <span className="text-xs font-semibold text-gray-700 dark:text-gray-200">{fmtDate(viewRecord.date)}</span>
                  </div>
                </div>
                ...
                {viewRecord.description && (
                  <div className="bg-gray-50 dark:bg-gray-800 rounded-xl px-4 py-3">
                    ...
                    <p className="text-sm text-gray-700 dark:text-gray-200 leading-relaxed whitespace-pre-wrap break-words">{viewRecord.description}</p>
                  </div>
                )}
                ...
                {vTags.length > 0 && (
                  <div className="flex flex-wrap gap-1.5">
                    {vTags.map((tag) => (
                      <span key={tag.id} className="px-2.5 py-1 rounded-full text-xs font-semibold text-white" style={{ backgroundColor: tag.color }}>{tag.name}</span>
                    ))}
                  </div>
                )}
              </div>
              ...
              {/* Fixed footer */}
              <div className="shrink-0 px-5 pb-6 pt-3 border-t border-gray-100 dark:border-gray-800">
                <button onClick={() => setViewRecord(null)} className="w-full py-3.5 rounded-xl bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 text-sm font-semibold">
                  Close
                </button>
              </div>
            </div>
